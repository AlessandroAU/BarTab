...
int main() {
    ...
    HKEY key{};
    if (RegCreateKeyExW(HKEY_CURRENT_USER,sandbox.c_str(),0,nullptr,0,KEY_ALL_ACCESS,nullptr,&key,nullptr)!=ERROR_SUCCESS) return 1;
    if (RegOverridePredefKey(HKEY_CURRENT_USER,key)!=ERROR_SUCCESS) { RegCloseKey(key); RegDeleteTreeW(HKEY_CURRENT_USER,sandbox.c_str()); return 1; }
    ...
    ok=ok && RegGetValueW(HKEY_CURRENT_USER,L"Software\\Microsoft\\Windows\\CurrentVersion\\Run",L"UsageTracker",RRF_RT_REG_SZ,nullptr,command,&bytes)==ERROR_SUCCESS;
