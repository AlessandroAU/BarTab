...
std::filesystem::path bundled(const std::filesystem::path& root, const wchar_t* name) {
    ...
    while (!error && it!=end && ++visited<=20000) {
        ...
        const bool wrong_arch=component.find(L"arm64")!=std::wstring::npos || component.find(L"aarch64")!=std::wstring::npos;
        if (it.depth()>=7 || wrong_arch || it->is_symlink(error)) it.disable_recursion_pending();
        error.clear();
        if (lower(it->path().filename().wstring())==name && file(it->path())) {
            const auto time=std::filesystem::last_write_time(it->path(),error);
            if (!error && (newest.empty() || time>modified)) { newest=it->path(); modified=time; }
        }
        error.clear(); it.increment(error);
    }
    return newest;
}
}
DiscoveryResult discover_executable(bool claude, const DiscoveryLocations& locations) {
    ...
    if (!locations.override_path.empty()) {
        if (lower(locations.override_path.extension().wstring())==L".exe" && file(locations.override_path)) return {locations.override_path,{}};
        return {{},provider+" override is not an existing .exe. Correct or remove USAGETRACKER_"+(claude ? "CLAUDE" : "CODEX")+"."};
    }
    for (const auto& bin : locations.bins) if (file(bin/name)) return {bin/name,{}};
    ...
    for (const auto& root : locations.extensions) {
        std::error_code error;
        std::filesystem::directory_iterator it(root,std::filesystem::directory_options::skip_permission_denied,error),end;
        while (!error && it!=end) {
            ...
            if (folder.find(prefix)==0 && folder.find(L"arm64")==std::wstring::npos) {
                ...
                if (!found.empty()) {
                    ...
                    if (!error && (newest.empty() || time>modified)) { newest=found; modified=time; }
                }
            }
            error.clear(); it.increment(error);
        }
    ...
    return {{},provider+" not found. Install its CLI or VS Code extension, then Refresh usage and detection."};
}
}
