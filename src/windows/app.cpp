...
App::App(bool smoke, bool live_test) : smoke_(smoke), demo_mode_(smoke), live_test_(live_test) {
    ...
    controller_ = CreateWindowExW(WS_EX_TOOLWINDOW,controller_class,L"UsageTracker controller",WS_OVERLAPPED,
        0,0,0,0,nullptr,nullptr,GetModuleHandleW(nullptr),this);
    if (!controller_) throw std::runtime_error("Could not create controller window");
    ...
    usage_.live = !smoke;
    if (!smoke) {
        usage_.account.installed=false;
        detect_providers();
        apply_providers();
    }
    taskbar_created_ = RegisterWindowMessageW(L"TaskbarCreated");
    add_tray();
    if (!SetTimer(controller_,1,500,nullptr)) throw std::runtime_error("Could not create update timer");
    log(L"Started native prototype; PID " + std::to_wstring(GetCurrentProcessId()));
}

App::~App() {
    if (controller_) KillTimer(controller_,1);
...
void App::register_class(const wchar_t* name, WNDPROC procedure) {
    ...
    cls.hIconSm = static_cast<HICON>(LoadImageW(cls.hInstance,MAKEINTRESOURCEW(IDI_USAGE_TRACKER),IMAGE_ICON,
        GetSystemMetrics(SM_CXSMICON),GetSystemMetrics(SM_CYSMICON),LR_SHARED));
    cls.lpszClassName = name;
    if (!RegisterClassExW(&cls)) throw std::runtime_error("Could not register window class");
}
...
LRESULT CALLBACK App::controller_proc(HWND window, UINT message, WPARAM w, LPARAM l) {
    ...
    if (app) {
        if (message==WM_SETTINGCHANGE || message==WM_THEMECHANGED || message==WM_SYSCOLORCHANGE) {
            app->tick();
            if (app->widget_) InvalidateRect(app->widget_,nullptr,FALSE);
            return 0;
        }
        if (message == WM_TIMER) {
            const bool codex_changed=app->codex_ && app->codex_->take(app->usage_.account);
            const bool claude_changed=app->claude_ && app->claude_->take(app->usage_.claude);
            if (codex_changed || claude_changed) {
                app->update_usage();
                if (IsWindowVisible(app->popup_)) app->render_details(app->details_pointer_);
            }
            app->tick(); return 0;
        }
        if (message == WM_CLOSE) { PostQuitMessage(0); return 0; }
        if (app->taskbar_created_ && message == app->taskbar_created_) { app->reset_widget(); app->add_tray(); return 0; }
        if (message == tray_message) {
            if (l == WM_LBUTTONUP) app->open_details();
            if (l == WM_RBUTTONUP || l == WM_CONTEXTMENU) app->show_menu();
            return 0;
        }
    }
...
void App::add_tray() {
    ...
    tray_.hIcon = static_cast<HICON>(LoadImageW(GetModuleHandleW(nullptr),MAKEINTRESOURCEW(IDI_USAGE_TRACKER),IMAGE_ICON,
        GetSystemMetrics(SM_CXSMICON),GetSystemMetrics(SM_CYSMICON),LR_SHARED));
    update_tooltip();
...
void App::update_tooltip() {
    ...
    if (usage_.live) {
        for (const auto& entry : {std::pair<const wchar_t*,const AccountUsage*>{L"Codex",&usage_.account},{L"Claude",&usage_.claude}}) {
            if (!(entry.second == &usage_.account ? usage_.codex_active() : usage_.claude_active())) continue;
            text+=L"\n"+std::wstring(entry.first)+L": ";
            if (!entry.second->error.empty()) text+=L"stale/unavailable ";
            for (const auto& window : entry.second->windows) text+=std::wstring(window.label.begin(),window.label.end())+L" "+std::to_wstring(window.remaining)+L"% ";
        }
    } else text=L"UsageTracker - demo data";
...
void App::show_menu() {
    ...
    AppendMenuW(menu,MF_STRING | (startup.enabled ? MF_CHECKED : MF_UNCHECKED) | (startup.error==ERROR_SUCCESS ? 0 : MF_GRAYED),6,L"Start at boot");
    AppendMenuW(menu,MF_STRING,3,L"Quit");
    POINT position{}; GetCursorPos(&position);
    SetForegroundWindow(controller_);
    const auto selected = TrackPopupMenu(menu,TPM_RETURNCMD | TPM_RIGHTBUTTON,position.x,position.y,0,controller_,nullptr);
    DestroyMenu(menu);
    PostMessageW(controller_,WM_NULL,0,0);
    if (selected == 3) PostQuitMessage(0);
    if (selected == 4) open_details(true);
    if (selected == 6) {
        const auto status=set_startup(!startup.enabled);
        if (status!=ERROR_SUCCESS) {
            wchar_t detail[512]{};
            FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,nullptr,static_cast<DWORD>(status),0,detail,static_cast<DWORD>(std::size(detail)),nullptr);
