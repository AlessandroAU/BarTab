...
LRESULT CALLBACK App::popup_proc(HWND window, UINT message, WPARAM w, LPARAM l) {
    ...
    if (app) {
        switch (message) {
        case WM_CLOSE: app->close_details(); return 0;
        case WM_ERASEBKGND: return 1;
        case WM_PAINT: app->paint_details(window); return 0;
        case WM_NCHITTEST: {
            POINT point{static_cast<short>(LOWORD(l)),static_cast<short>(HIWORD(l))};
            ScreenToClient(window,&point);
            if (point.y >= 0 && point.y < MulDiv(48,GetDpiForWindow(window),96)) return HTCAPTION;
            return HTCLIENT;
        }
        case WM_MOUSEWHEEL: case WM_MOUSEMOVE: case WM_MOUSELEAVE:
        case WM_LBUTTONDOWN: case WM_LBUTTONUP: case WM_KEYDOWN:
        case WM_CAPTURECHANGED: case WM_CANCELMODE:
            app->details_event(window,message,w,l); return 0;
        case WM_SIZE:
            round_window(window);
            if (app->popup_ && IsWindowVisible(window)) app->render_details({});
            return 0;
        case WM_DPICHANGED: {
...
void App::close_details() {
    ...
    hide_hover();
    if (settings_preview_) {
        settings_preview_ = false;
        apply_preferences(original_preferences_);
    }
}

void App::render_details(ClayWidgets_Input input) {
    if (!popup_) return;
    RECT rect{}; GetClientRect(popup_,&rect);
    if (rect.right <= 0 || rect.bottom <= 0) return;
    const float scale = popup_scale();
    renderer_.set_scale(scale);
    if (details_scale_ != scale) { details_view_.invalidate_measurements(); details_scale_ = scale; }
    auto frame = details_view_.frame(usage_,input,static_cast<float>(rect.right)/scale,static_cast<float>(rect.bottom)/scale);
    if (frame.close) { close_details(); return; }
    if (frame.refresh) { detect_providers(); frame.changed = true; if (codex_) codex_->refresh(); if (claude_) claude_->refresh(); }
    if (frame.save) {
    ...
    if (frame.changed) {
        if (settings_mode_) apply_preferences(details_view_.preferences());
        else update_usage();
        // Settle the borrowed percentage labels after the slider changes data.
        frame = details_view_.frame(usage_,details_pointer_,static_cast<float>(rect.right)/scale,static_cast<float>(rect.bottom)/scale);
    }
    details_pixels_ = renderer_.render(frame.commands,rect.right,rect.bottom,scale,false);
    ++details_frames_;
    SetCursor(LoadCursorW(nullptr,details_view_.cursor() == CLAY_WIDGETS_CURSOR_POINTER ? IDC_HAND : IDC_ARROW));
...
void App::paint_pixels(HWND window, const ui::Pixels& pixels) {
    ...
    if (!pixels.data.empty()) {
        ...
        SetDIBitsToDevice(dc,0,0,pixels.width,pixels.height,0,0,0,
            static_cast<UINT>(pixels.height),pixels.data.data(),&info,DIB_RGB_COLORS);
    }
...
void App::details_event(HWND window, UINT message, WPARAM w, LPARAM l) {
    ...
    if (message == WM_MOUSELEAVE && GetCapture() == window) return;
    ...
    if (message == WM_MOUSEMOVE || message == WM_LBUTTONDOWN || message == WM_LBUTTONUP) {
        ...
        if (message == WM_MOUSEMOVE && input.mouseX == details_pointer_.mouseX && input.mouseY == details_pointer_.mouseY) return;
    ...
    if (message == WM_MOUSEWHEEL) {
        POINT point{static_cast<short>(LOWORD(l)),static_cast<short>(HIWORD(l))};
        ScreenToClient(window,&point);
        input.mouseX = point.x/popup_scale(); input.mouseY = point.y/popup_scale();
        input.scrollY = static_cast<short>(HIWORD(w))/static_cast<float>(WHEEL_DELTA);
    }
    if (message == WM_MOUSELEAVE) input.mouseX = input.mouseY = -100;
    ...
    if (message == WM_KEYDOWN) {
        ...
        input.keyLeft = w == VK_LEFT; input.keyRight = w == VK_RIGHT;
        input.keyUp = w == VK_UP; input.keyDown = w == VK_DOWN;
        input.keyHome = w == VK_HOME; input.keyEnd = w == VK_END;
        input.keyEnter = w == VK_RETURN; input.keySpace = w == VK_SPACE;
...
void App::open_details(bool settings) {
    ...
    if (!popup_) {
        popup_ = CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,popup_class,L"UsageTracker - demo",
            WS_POPUP,CW_USEDEFAULT,CW_USEDEFAULT,360,370,controller_,nullptr,GetModuleHandleW(nullptr),this);
        if (!popup_) return;
    }
    if (IsWindowVisible(popup_) && settings_mode_ == settings) { SetForegroundWindow(popup_); return; }
    if (IsWindowVisible(popup_)) close_details();
    settings_mode_ = settings;
    settings_preview_ = settings;
    original_preferences_ = preferences_;
    detect_providers();
    ...
    if (!dpi) dpi = 96;
    RECT size{0,0,MulDiv(usage_.live ? 1120 : 400,static_cast<int>(dpi),96),MulDiv(usage_.live ? 700 : 470,static_cast<int>(dpi),96)};
    ...
    const int x = settings ? monitor.rcWork.left+(monitor.rcWork.right-monitor.rcWork.left-width)/2 : std::max(static_cast<int>(monitor.rcWork.left),std::min(anchor.right()-width,static_cast<int>(monitor.rcWork.right)-width));
    const int y = settings ? monitor.rcWork.top+(monitor.rcWork.bottom-monitor.rcWork.top-height)/2 : std::max(static_cast<int>(monitor.rcWork.top),std::min(anchor.y-height-8,static_cast<int>(monitor.rcWork.bottom)-height));
