...
Rect find_space(Rect panel, const std::vector<Rect>& occupied, int width, int height, int gap,
    ...
    for (const auto& block : blocks) {
        ...
        if (end - cursor >= width) gaps.emplace_back(cursor, end);
        cursor = std::max(cursor, block.right() + gap);
    }
    if (panel.right() - gap - cursor >= width) gaps.emplace_back(cursor, panel.right() - gap);
    ...
    for (const auto& candidate : gaps) {
        ...
        if (distance < best) { best = distance; x = reachable; }
    }
