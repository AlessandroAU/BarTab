...
Clay_String string(const char* text) { return {false, static_cast<int32_t>(std::strlen(text)), text}; }
Clay_ElementId id(const char* text) { return Clay_GetElementId(string(text)); }
Clay_Color color(Color value) { return {static_cast<float>(value.r),static_cast<float>(value.g),static_cast<float>(value.b),255}; }
...
std::string date(std::int64_t timestamp) {
    if (timestamp <= 0) return std::string("Unavailable");
    const auto time = static_cast<std::time_t>(timestamp);
    std::tm local{};
#ifdef _WIN32
    if (localtime_s(&local,&time)) return std::string("Unavailable");
#else
    if (!localtime_r(&time,&local)) return std::string("Unavailable");
#endif
    char buffer[40]{};
...
std::string reset_time(std::int64_t timestamp, bool date_only = false, bool day_key = false) {
    ...
    std::tm local{};
#ifdef _WIN32
    if (localtime_s(&local,&time)) return "?";
#else
    if (!localtime_r(&time,&local)) return "?";
#endif
    char buffer[24]{};
...
Clay_Color View::provider_color(const char* name, bool secondary) const {
    if (std::strstr(name,"Claude")) return secondary ? Clay_Color{232,171,145,255} : Clay_Color{217,119,87,255};
    if (std::strstr(name,"Codex")) return surface_==Surface::Widget && system_light_ ? Clay_Color{35,35,40,255} : Clay_Color{236,236,241,255};
    return accent_color();
}
void View::wrapped_text(const std::string& value, uint16_t size, Clay_Color tint) {
    Clay_TextElementConfig config{};
    config.fontSize=size; config.textColor=tint; config.wrapMode=CLAY_TEXT_WRAP_WORDS;
    CLAY_TEXT(string(label(value)),config);
}
bool View::setting_slider(const char* name, const char* title, int& value, int min, int max, int step, const char* unit) {
    text(label(std::string(title)+": "+std::to_string(value)+unit),settings_body,{210,220,234,255});
    float current=static_cast<float>(value);
    ClayWidgets_SliderOptions options{};
    options.minValue=static_cast<float>(min); options.maxValue=static_cast<float>(max); options.step=static_cast<float>(step);
    if (!ClayWidgets_Slider(widgets_.get(),id(name),&current,options)) return false;
    value=static_cast<int>(std::lround(current)); return true;
}
bool View::choice_dropdown(const char* name, const char* title, const char* const* choices, int count, int& value) {
    std::vector<Clay_String> items;
    for (int i=0; i<count; ++i) items.push_back(string(choices[i]));
    int32_t selected=value;
    text(title,settings_body,{210,220,234,255});
    if (!ClayWidgets_Combo(widgets_.get(),id(name),CLAY_STRING(""),items.data(),count,&selected)) return false;
...
bool View::interval_dropdown(const char* name, int& seconds) {
    ...
    if (std::find(values.begin(),values.end(),seconds)==values.end()) {
        values.push_back(seconds); std::sort(values.begin(),values.end());
    }
    std::vector<Clay_String> items;
    for (const auto value : values) {
        const auto caption=value%60==0 ? std::to_string(value/60)+(value==60 ? " minute" : " minutes") : std::to_string(value)+" seconds";
        items.push_back(string(label(caption)));
    }
    int32_t selected=static_cast<int32_t>(std::find(values.begin(),values.end(),seconds)-values.begin());
    text("Update interval",settings_body,{210,220,234,255});
    if (!ClayWidgets_Combo(widgets_.get(),id(name),CLAY_STRING(""),items.data(),static_cast<int32_t>(items.size()),&selected)) return false;
...
void View::settings_panel(const Usage& data, Frame& result, ClayWidgets_Input input) {
    ...
    CLAY(CLAY_ID("SettingsPanel"),root) {
        auto heading=column(0,12); heading.layout.layoutDirection=CLAY_LEFT_TO_RIGHT;
        ...
        CLAY(CLAY_ID("ConfigurationPanels"),panels) {
            ...
            CLAY(CLAY_ID("AppearancePanel"),appearance) {
                ...
                scroll.width=CLAY_SIZING_GROW(0); scroll.height=CLAY_SIZING_GROW(0);
                scroll.padding=12; scroll.childGap=10; scroll.fadeMargin=8;
                ClayWidgets_BeginScrollPanel(widgets_.get(),CLAY_ID("AppearanceScroll"),scroll);
                auto& a=preferences_.appearance;
                result.changed=setting_slider("TextSizeSlider","Text size",a.text_percent,100,300,10,"%") || result.changed;
                ...
                if (ClayWidgets_Button(widgets_.get(),CLAY_ID("AccentChoice"),string(accents[a.accent]))) { a.accent=(a.accent+1)%3; result.changed=true; }
            ...
            CLAY(CLAY_ID("ProvidersPanel"),providers) {
                ...
                ClayWidgets_ScrollPanelOptions scroll{};
                scroll.width=CLAY_SIZING_GROW(0); scroll.height=CLAY_SIZING_GROW(0);
                scroll.padding=12; scroll.childGap=12; scroll.fadeMargin=8;
                ClayWidgets_BeginScrollPanel(widgets_.get(),CLAY_ID("ProvidersScroll"),scroll);
                auto cards=column(0,14); cards.layout.layoutDirection=CLAY_LEFT_TO_RIGHT;
                CLAY(CLAY_ID("ProviderCards"),cards) {
                    for (int index=0; index<2; ++index) {
                        const bool is_codex=index==0;
                        const auto& account=is_codex ? data.account : data.claude;
                        auto card=column(12,10);
                        card.backgroundColor=background_color(); card.cornerRadius=CLAY_CORNER_RADIUS(6);
                        CLAY(id(is_codex ? "LiveUsageCodex" : "LiveUsageClaude"),card) {
                            ...
                            result.changed=ClayWidgets_Checkbox(widgets_.get(),id(is_codex ? "EnableCodex" : "EnableClaude"),string(is_codex ? "Enable Codex" : "Enable Claude"),&enabled) || result.changed;
                            text(account.installed ? (enabled ? (account.error.empty() ? "Detected / tracking" : "Detected / refresh failed") : "Detected / disabled") : "Not detected",settings_body,account.installed ? accent_color() : Clay_Color{240,180,90,255});
                            auto& interval=is_codex ? preferences_.codex_interval : preferences_.claude_interval;
                            result.changed=interval_dropdown(is_codex ? "CodexInterval" : "ClaudeInterval",interval) || result.changed;
                            wrapped_text(account.updated ? "Last update: "+date(account.updated)+" (local)" : "Last update: Never",settings_body,{166,187,208,255});
                            wrapped_text("Plan: "+(account.plan.empty() ? std::string("Not reported") : account.plan),settings_body,{210,220,234,255});
                            text("Executable",settings_body,{166,187,208,255});
                            if (account.executable_path.empty()) text("Not found",settings_body,{166,187,208,255});
                            else {
                                ...
                                for (std::size_t offset=0; offset<account.executable_path.size();) {
                                    ...
                                    while (end<account.executable_path.size() && (static_cast<unsigned char>(account.executable_path[end])&0xc0)==0x80) ++end;
                                    text(label(account.executable_path.substr(offset,end-offset)),settings_body,{166,187,208,255}); offset=end;
                                }
                            }
                            if (!account.error.empty()) wrapped_text("Last error: "+account.error,settings_body,{240,180,90,255});
                            if (!enabled && !account.windows.empty()) wrapped_text("Retained reading; updates paused.",settings_body,{240,180,90,255});
                            if (account.windows.empty()) wrapped_text(account.installed ? "No usage reading yet." : "Install and sign in, then refresh detection.",settings_body,{166,187,208,255});
                            for (std::size_t i=0; i<account.windows.size(); ++i) {
                                ...
                                compact_bar(label(std::string(is_codex ? "SettingsCodex" : "SettingsClaude")+std::to_string(i)),"",window.remaining,label(std::to_string(window.remaining)+"%"));
                                wrapped_text(std::to_string(window.remaining)+"% remaining / "+std::to_string(100-window.remaining)+"% used",settings_body,{166,187,208,255});
                                wrapped_text("Resets: "+date(window.resets_at)+" (local)",settings_body,{166,187,208,255});
                            }
                        }
                    }
                }
        ...
        auto actions=column(0,12); actions.layout.layoutDirection=CLAY_LEFT_TO_RIGHT;
        actions.layout.childAlignment.y=CLAY_ALIGN_Y_CENTER;
        CLAY(CLAY_ID("SettingsActions"),actions) {
            if (ClayWidgets_Button(widgets_.get(),CLAY_ID("ResetAppearance"),CLAY_STRING("Reset appearance to defaults"))) { preferences_.appearance=Appearance{}; result.changed=true; }
...
void View::set_text_percent(int value) { preferences_.appearance.text_percent = std::clamp(value,100,300); }
void View::text(const char* value, uint16_t size, Clay_Color tint) {
    Clay_TextElementConfig config{};
    config.fontSize = static_cast<uint16_t>(std::lround(size * (surface_ == Surface::Widget || surface_ == Surface::Hover ? preferences_.appearance.text_percent/100.f : 1.f)));
    if (surface_ == Surface::Widget || surface_ == Surface::Hover) config.fontId = static_cast<uint16_t>(preferences_.appearance.font);
...
View::~View() {
    if (Clay_GetCurrentContext() == clay_) Clay_SetCurrentContext(nullptr);
    std::free(arena_);
}
...
ClayWidgets_Cursor View::cursor() const { return ClayWidgets_GetCursor(widgets_.get()); }
Clay_BoundingBox View::bounds(const char* name) {
    Clay_SetCurrentContext(clay_);
    return Clay_GetElementData(id(name)).boundingBox;
}
void View::compact_bar(const char* name, const char* label, int value, std::string_view percent, bool aligned, float percent_width) {
    ...
    CLAY(id(name),row) {
        if (surface_ == Surface::Hover || aligned) {
            ...
            float label_width=90.f*preferences_.appearance.text_percent/100.f;
            if (aligned) {
                const char* longest=std::strchr(label,'*') ? "General *" : "General";
                Clay_TextElementConfig config{};
                config.fontId=static_cast<uint16_t>(preferences_.appearance.font);
                config.fontSize=static_cast<uint16_t>(std::lround(text_size*preferences_.appearance.text_percent/100.f));
                const Clay_StringSlice sample{static_cast<int32_t>(std::strlen(longest)),longest,longest};
                label_width=std::ceil(widgets_->measureText(sample,&config,widgets_->measureTextUserData).width);
            }
            name_column.layout.sizing.width = CLAY_SIZING_FIXED(label_width);
            CLAY_AUTO_ID(name_column) { text(label,text_size,{172,190,210,255}); }
        } else text(label,text_size,{172,190,210,255});
        ...
        CLAY(id(this->label(std::string(name)+"Track")),track) {
            ...
            fill.backgroundColor = branded ? provider_color(name,std::strstr(name,"Fable") || std::strstr(label,"Fable")) : value > 30 ? accent_color() : color(bar_color(value));
            fill.cornerRadius = CLAY_CORNER_RADIUS(static_cast<float>(preferences_.appearance.corner_radius));
            CLAY_AUTO_ID(fill) {}
        }
        if (surface_ == Surface::Hover || aligned) {
            auto value_column = column(0,0);
            value_column.layout.sizing.width = CLAY_SIZING_FIXED(aligned ? percent_width : 28.f*preferences_.appearance.text_percent/100.f);
            value_column.layout.childAlignment.x = aligned ? CLAY_ALIGN_X_LEFT : CLAY_ALIGN_X_RIGHT;
            CLAY_AUTO_ID(value_column) { text(percent.data(),text_size,{236,243,250,255}); }
        } else text(percent.data(),text_size,{236,243,250,255});
    }
}
void View::claude_only(const AccountUsage& account, const Allowance& weekly, const Allowance& fable) {
    ...
    const float percent_width=std::ceil(std::max(measure_percent(general_percent),measure_percent(fable_percent)));
    auto row=column(0,6);
    row.layout.layoutDirection=CLAY_LEFT_TO_RIGHT;
    row.layout.childAlignment.y=CLAY_ALIGN_Y_CENTER;
    CLAY(CLAY_ID("ClaudeOnly"),row) {
        auto bars=column(0,2);
        if (preferences_.appearance.text_percent>160) { bars.layout.layoutDirection=CLAY_LEFT_TO_RIGHT; bars.layout.childGap=12; }
        CLAY(CLAY_ID("ClaudeOnlyBars"),bars) {
            compact_bar("ClaudeGeneral",account.error.empty() ? "General" : "General *",weekly.remaining,label(general_percent),true,percent_width);
            compact_bar("ClaudeFable",account.error.empty() ? "Fable" : "Fable *",fable.remaining,label(fable_percent),true,percent_width);
        }
        if (preferences_.appearance.show_resets) {
            const bool shared=weekly.resets_at>0 && fable.resets_at>0 && reset_time(weekly.resets_at,true,true)==reset_time(fable.resets_at,true,true);
            ...
            CLAY(CLAY_ID("ClaudeResetColumn"),resets) {
                ...
                const auto size=static_cast<uint16_t>(std::min(9.f,1600.f/preferences_.appearance.text_percent));
...
void View::taskbar_allowance(const char* name, const char* title, const Allowance& allowance, bool stale, bool show_reset, bool date_only) {
    auto row = column(0,8);
    row.layout.layoutDirection = CLAY_LEFT_TO_RIGHT;
    row.layout.childAlignment.y = CLAY_ALIGN_Y_CENTER;
    CLAY_AUTO_ID(row) {
        compact_bar(name,label(std::string(title)+(stale ? " *" : "")),allowance.remaining,label(std::to_string(allowance.remaining)+"%"));
        if (preferences_.appearance.show_resets && show_reset) text(label(std::string(std::strcmp(name,"Codex")==0 ? "" : "reset ")+reset_time(allowance.resets_at,date_only || preferences_.appearance.widget_width<208)),9,{166,187,208,255});
    }
}
    ...
    CLAY(CLAY_ID("Claude"),row) {
        ...
        CLAY(CLAY_ID("ClaudeTracks"),tracks) {
            for (int i=0; i<2; ++i) {
                Clay_ElementDeclaration track{};
                track.layout.sizing.width=CLAY_SIZING_GROW(0);
                track.layout.sizing.height=CLAY_SIZING_FIXED(1.5f*static_cast<float>(std::max(1,(preferences_.appearance.bar_height-1)/2)));
                track.backgroundColor=widgets_->theme.borderColor;
                track.cornerRadius=CLAY_CORNER_RADIUS(static_cast<float>(preferences_.appearance.corner_radius));
                CLAY(id(i==0 ? "ClaudeWeeklyTrack" : "ClaudeFableTrack"),track) {
                    Clay_ElementDeclaration fill{};
                    fill.layout.sizing.width=CLAY_SIZING_PERCENT((i==0 ? weekly.remaining : fable.remaining)/100.f);
                    fill.layout.sizing.height=CLAY_SIZING_GROW(0);
        ...
        text(label(std::to_string(weekly.remaining)+(narrow ? "/" : " / ")+std::to_string(fable.remaining)+"%"),10,{236,243,250,255});
        if (preferences_.appearance.show_resets) {
            auto resets=reset_time(weekly.resets_at,true);
            if (weekly.resets_at<=0 || fable.resets_at<=0 || reset_time(weekly.resets_at,true,true)!=reset_time(fable.resets_at,true,true))
...
Clay_Dimensions widget_size(const Usage& data, const Appearance& appearance) {
    ...
    if (data.live && appearance.text_percent>160) {
        if (data.codex_active() && data.claude_active()) rows=2;
        else if (data.claude_active()) {
            const auto& windows=data.claude.windows;
            const bool weekly=std::any_of(windows.begin(),windows.end(),[](const auto& w) { return w.label=="Weekly"; });
            const bool fable=std::any_of(windows.begin(),windows.end(),[](const auto& w) { return w.label=="Fable weekly"; });
            if (weekly && fable) rows=2;
        }
    }
    return {appearance.widget_width*appearance.text_percent/100.f*rows,static_cast<float>(widget_height)};
}
Clay_Dimensions hover_size(const Usage& data, int text_percent) {
    const float scale = text_percent/100.f;
    if (!data.live) return {320.f*scale,250.f*scale};
    ...
    int providers = 0;
    for (const auto* account : {&data.account,&data.claude}) {
        if (!(account == &data.account ? data.codex_active() : data.claude_active())) continue;
        ++providers;
        height += 10 + font(12);
        height += account->windows.empty() ? 6 + font(11) :
            (2*font(10)+9)*static_cast<float>(account->windows.size());
    }
    if (!providers) height += 10 + font(12);
...
void View::hover_usage(const Usage& data) {
    ...
    CLAY(CLAY_ID("HoverOverview"),root) {
        ...
        for (const auto& entry : {std::pair<const char*,const AccountUsage*>{"Codex",&data.account},{"Claude",&data.claude}}) {
            const auto& account = *entry.second;
            if (!(entry.second == &data.account ? data.codex_active() : data.claude_active())) continue;
            auto provider = column(0,6);
            CLAY(id(label(std::string("Hover")+entry.first)),provider) {
                text(label(std::string(entry.first)+(account.error.empty() ? "" : account.windows.empty() ? " - unavailable" : " - stale")),12,
                    account.error.empty() ? provider_color(entry.first) : Clay_Color{240,180,90,255});
                if (account.windows.empty()) text(account.error.empty() ? "Connecting..." : "Refresh failed. Click for details.",11,{157,174,193,255});
                for (std::size_t i=0; i<account.windows.size(); ++i) {
                    const auto& window = account.windows[i];
                    auto allowance = column(0,3);
                    CLAY_AUTO_ID(allowance) {
                        compact_bar(label(std::string("Hover")+entry.first+std::to_string(i)),window.label.c_str(),window.remaining,label(std::to_string(window.remaining)+"%"));
                        text(label(window.resets_at > 0 ? "Resets "+date(window.resets_at) : "Reset unavailable"),10,{157,174,193,255});
                    }
                }
            }
        }
        if (!data.codex_active() && !data.claude_active()) text(!data.codex_enabled && !data.claude_enabled ? "Providers disabled - open settings" : "No supported installations detected",12,{157,174,193,255});
        text("Click for settings and usage",10,{157,174,193,255});
    }
}
void View::live_usage(const char* provider, const AccountUsage& account, Frame& result, ClayWidgets_Input input) {
    const bool compact = surface_ == Surface::Widget;
    const bool stale = !account.error.empty();
    auto root = column(compact ? 6 : 16,compact ? 3 : 12);
    root.layout.sizing.height = CLAY_SIZING_GROW(0);
    if (compact) root.layout.padding.top = root.layout.padding.bottom = 3;
    else root.backgroundColor = background_color();
    CLAY(id(label(std::string("LiveUsage")+provider)),root) {
        text(label(std::string(provider)+(compact ? stale ? "   STALE" : "   REMAINING" : " usage")),compact ? 9 : 18,
            stale ? Clay_Color{240,180,90,255} : hovered_ ? accent_color() : Clay_Color{166,187,208,255});
        if (account.windows.empty()) {
            text(stale ? "Usage unavailable" : label(std::string("Connecting to ")+provider+"..."),compact ? 10 : 14,{236,243,250,255});
        }
        auto windows = column(0,compact ? 10 : 12);
        if (compact) windows.layout.layoutDirection = CLAY_LEFT_TO_RIGHT;
        CLAY_AUTO_ID(windows) {
            for (std::size_t i=0; i<account.windows.size(); ++i) {
                ...
                if (compact) compact_bar(name,window.label.c_str(),window.remaining,percent);
                else {
                    ...
                    CLAY_AUTO_ID(card) {
                        ...
                        text(label(std::to_string(window.remaining)+"% remaining / "+std::to_string(100-window.remaining)+"% used"),11,{157,174,193,255});
                        text(label("Resets: "+date(window.resets_at)+" (local)"),11,{157,174,193,255});
                    }
                }
            }
        }
        if (!compact) {
            if (stale) text("Refresh failed - last reading may be stale",11,{240,180,90,255});
            if (!account.updated || surface_ == Surface::Details || surface_ == Surface::Settings) {
                if (stale) text(label(account.error.substr(0,50)),11,{240,180,90,255});
            }
            if (account.updated) text(label("Updated: "+date(account.updated)+" (local)"),11,{157,174,193,255});
            if (surface_ == Surface::Details || surface_ == Surface::Settings) {
                if (!account.plan.empty()) text(label("Plan: "+account.plan),12,{157,174,193,255});
            }
            if (surface_ == Surface::Details) {
                text("Refreshes every minute. Allowance is not a token count.",11,{157,174,193,255});
                result.refresh=ClayWidgets_Button(widgets_.get(),id(label(std::string(provider)+"RefreshUsage")),CLAY_STRING("Refresh now")) || result.refresh;
                result.close=ClayWidgets_Button(widgets_.get(),id(label(std::string(provider)+"Close")),CLAY_STRING("Close")) || input.keyEscape || result.close;
            }
        }
    }
}
Frame View::frame(Usage& data, ClayWidgets_Input input, float width, float height) {
    ...
    widgets_->theme.accentMutedColor={selected_accent.r*0.35f,selected_accent.g*0.35f,selected_accent.b*0.35f,255};
    ...
    if (surface_==Surface::Widget && system_light_) widgets_->theme.borderColor={180,187,196,255};
    ...
    if (data.live && surface_ == Surface::Settings) {
        settings_panel(data,result,input);
    } else if (data.live && surface_ == Surface::Hover) {
        hover_usage(data);
    } else if (data.live && surface_ != Surface::Settings) {
        if (surface_ == Surface::Widget && (data.codex_active() || data.claude_active())) {
            ...
            if (horizontal) root.layout.layoutDirection=CLAY_LEFT_TO_RIGHT;
            CLAY(CLAY_ID("Providers"),root) {
                for (const auto& entry : {std::pair<const char*,const AccountUsage*>{"Codex",&data.account},{"Claude",&data.claude}}) {
                    const auto& account=*entry.second;
                    if (!(entry.second == &data.account ? data.codex_active() : data.claude_active())) continue;
                    if (entry.second==&data.claude) {
                        const auto weekly=std::find_if(account.windows.begin(),account.windows.end(),[](const auto& w) { return w.label=="Weekly"; });
                        const auto fable=std::find_if(account.windows.begin(),account.windows.end(),[](const auto& w) { return w.label=="Fable weekly"; });
                        if (weekly!=account.windows.end() && fable!=account.windows.end()) {
                            if (!data.codex_active()) {
                                claude_only(account,*weekly,*fable);
                            } else claude_bars(account,*weekly,*fable);
                            continue;
                        }
                    }
                    const auto lowest=std::min_element(account.windows.begin(),account.windows.end(),[](const auto& a,const auto& b) { return a.remaining<b.remaining; });
                    if (lowest==account.windows.end()) text(label(std::string(entry.first)+(account.error.empty() ? ": connecting..." : ": unavailable")),10,{240,180,90,255});
                    else if (entry.second==&data.account && !data.claude_active() && preferences_.appearance.show_resets) {
                        auto codex=column(0,2);
                        CLAY(CLAY_ID("CodexOnly"),codex) {
                            // Keep both lines legible inside the taskbar's fixed height.
                            const int percent=preferences_.appearance.text_percent;
                            preferences_.appearance.text_percent=std::min(percent,160);
                            compact_bar("Codex",account.error.empty() ? "Codex" : "Codex *",lowest->remaining,label(std::to_string(lowest->remaining)+"%"));
                            ...
                            preferences_.appearance.text_percent=percent;
                        }
                    }
                    else taskbar_allowance(entry.first,entry.first,*lowest,!account.error.empty());
                }
            }
            ...
            CLAY(CLAY_ID("NoProviders"),root) {
                ...
                text(!data.codex_enabled && !data.claude_enabled ? (narrow ? "Providers disabled" : "Providers disabled - open settings") :
                    (narrow ? "No providers detected" : "No supported installations detected"),surface_==Surface::Widget ? 9 : 14,{166,187,208,255});
                if (surface_==Surface::Details) {
                    text("Install Codex or Claude Code, then refresh.",12,{166,187,208,255});
                    result.refresh=ClayWidgets_Button(widgets_.get(),CLAY_ID("Detect"),CLAY_STRING("Refresh now"));
                    result.close=ClayWidgets_Button(widgets_.get(),CLAY_ID("Close"),CLAY_STRING("Close")) || input.keyEscape;
                }
            }
        } else {
            auto row=column(0,8);
            ...
            CLAY(CLAY_ID("ProviderColumns"),row) {
                if (data.codex_active()) live_usage("Codex",data.account,result,input);
                if (data.claude_active()) live_usage("Claude",data.claude,result,input);
            }
        }
    } else if (surface_ == Surface::Widget) {
        auto root = column(6,2);
        root.layout.padding.top = 2;
        root.layout.padding.bottom = 2;
        root.layout.sizing.height = CLAY_SIZING_GROW(0);
        CLAY(CLAY_ID("Widget"),root) {
            if (preferences_.appearance.text_percent<=160) text("DEMO   REMAINING",9,hovered_ ? accent_color() : Clay_Color{166,187,208,255});
    ...
    } else if (surface_ == Surface::Hover) {
        ...
        CLAY(CLAY_ID("Hover"),root) {
            ...
            for (int index = 0; index < 2; ++index) {
                ...
                card.cornerRadius = CLAY_CORNER_RADIUS(static_cast<float>(preferences_.appearance.corner_radius));
                CLAY_AUTO_ID(card) {
                    compact_bar(index == 0 ? "HoverSession" : "HoverWeekly",
                        index == 0 ? "Session" : "Weekly",index == 0 ? data.session() : data.weekly(),index == 0 ? session_ : weekly_);
                    auto row = column(0,8);
                    row.layout.layoutDirection = CLAY_LEFT_TO_RIGHT;
                    CLAY_AUTO_ID(row) {
    ...
    } else if (surface_ == Surface::Settings) {
        ...
        CLAY(CLAY_ID("SettingsPanel"),panel) {
            auto root = column(24,data.live ? 12 : 18);
            if (data.live) root.layout.sizing.width = CLAY_SIZING_FIXED(320);
            root.layout.sizing.height = CLAY_SIZING_GROW(0);
            CLAY(CLAY_ID("Settings"),root) {
                ...
                ClayWidgets_SliderOptions options{};
                options.minValue = 100; options.maxValue = 300; options.step = 10;
                ...
                if (ClayWidgets_Button(widgets_.get(),CLAY_ID("ResetTextSize"),CLAY_STRING("Reset to default"))) {
                    set_text_percent(150); result.changed = true;
                }
                if (data.live) {
                    text("Providers",17,{236,243,250,255});
                    result.changed = ClayWidgets_Checkbox(widgets_.get(),CLAY_ID("EnableCodex"),CLAY_STRING("Enable Codex"),&preferences_.codex_enabled) || result.changed;
                    result.changed = ClayWidgets_Checkbox(widgets_.get(),CLAY_ID("EnableClaude"),CLAY_STRING("Enable Claude"),&preferences_.claude_enabled) || result.changed;
                }
                Clay_ElementDeclaration space{};
                ...
                result.save = ClayWidgets_Button(widgets_.get(),CLAY_ID("SaveSettings"),CLAY_STRING("Save settings"));
                result.close = ClayWidgets_Button(widgets_.get(),CLAY_ID("CancelSettings"),CLAY_STRING("Cancel")) || input.keyEscape;
                if (data.live) result.refresh = ClayWidgets_Button(widgets_.get(),CLAY_ID("RefreshUsage"),CLAY_STRING("Refresh usage"));
            }
            if (data.live) {
                auto usage = column(8,12);
                usage.layout.sizing.height = CLAY_SIZING_GROW(0);
                CLAY(CLAY_ID("SettingsUsage"),usage) {
                    text("Usage and connections",22,{236,243,250,255});
                    text("Refreshes every minute. Reset times are local.",12,{157,174,193,255});
                    auto providers = column(0,8);
                    providers.layout.layoutDirection = CLAY_LEFT_TO_RIGHT;
                    providers.layout.sizing.height = CLAY_SIZING_GROW(0);
                    CLAY(CLAY_ID("SettingsProviders"),providers) {
                        if (data.codex_active()) live_usage("Codex",data.account,result,input);
                        if (data.claude_active()) live_usage("Claude",data.claude,result,input);
                        if (!data.codex_active() && !data.claude_active()) {
                            auto empty = column(16,12);
                            CLAY(CLAY_ID("SettingsNoProviders"),empty) {
                                text(!data.codex_enabled && !data.claude_enabled ? "All providers disabled" : "No supported installations detected",14,{166,187,208,255});
                                text(!data.codex_enabled && !data.claude_enabled ? "Enable a provider and save settings to resume." : "Install and sign in to Codex or Claude Code, then refresh.",12,{166,187,208,255});
                            }
                        }
                    }
                }
            ...
            for (int index = 0; index < 2; ++index) {
                ...
                CLAY(id(index == 0 ? "SessionCard" : "WeeklyCard"),card) {
                    ...
                    CLAY_AUTO_ID(row) {
                        ...
                        spacer.layout.sizing.width = CLAY_SIZING_GROW(0);
                        CLAY_AUTO_ID(spacer) {}
                        text((index == 0 ? session_ : weekly_).c_str(),17,color(bar_color(index == 0 ? data.session() : data.weekly())));
                    }
                    ...
                    options.step = 1;
                    if (ClayWidgets_Slider(widgets_.get(),id(index == 0 ? "SessionSlider" : "WeeklySlider"),&value,options)) {
                        if (index == 0) data.set_session(static_cast<int>(std::lround(value)));
                        else data.set_weekly(static_cast<int>(std::lround(value)));
                        result.changed = true;
                    }
    ...
    if (dismiss_dropdown) result.close = false;
